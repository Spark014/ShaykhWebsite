...
const basePageTexts = {
  ...
  ar: {
    title: "إرسال سؤال",
    description: "أرسل سؤالك. يرجى تقديم عنوان بريد إلكتروني صحيح. يتم حفظ الأسئلة في قاعدة بياناتنا لمراجعتها من قبل الشيخ. يجب أن يكون سؤالك باللغة العربية.",
    ...
    toastDescriptionSuccess: "تم إرسال سؤالك بنجاح. جزاك الله خيرًا.",
    toastErrorTitle: "خطأ في الإرسال",
    toastDescriptionErrorGeneric: "لم نتمكن من إرسال سؤالك. يرجى المحاولة مرة أخرى لاحقاً.",
    supabaseError: "حدث خطأ في قاعدة البيانات. يرجى التأكد من ملء النموذج بشكل صحيح والمحاولة مرة أخرى. إذا استمرت المشكلة، قد تكون الخدمة غير متاحة مؤقتًا أو هناك خطأ في الإعدادات.",
    validation: {
    ...
    try {
      ...
      if (pgError.hint) {
          ...
          if (pgError.hint.toLowerCase().includes("rls") || pgError.hint.toLowerCase().includes("policy")) {
              ...
              if (pgError.message.includes('question_text_min_length')) {
                toastDisplayMessage = T.validation.questionMinLength;
              } else if (pgError.message.includes('question_text_check')) { // Assuming your Arabic-only check constraint name involves 'question_text_check'
      ...
      if (pgError.code) {
          ...
          if (pgError.code === '23514') { // Check constraint violation code
            ...
            if (pgError.message.includes('question_text_min_length')) {
                toastDisplayMessage = T.validation.questionMinLength;
            } else if (pgError.message.includes('question_text_check')) { // Adapt if your Arabic-only constraint name is different
                toastDisplayMessage = T.validation.questionArabicOnly;
            } else {
