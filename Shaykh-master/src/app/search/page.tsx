...
import { Home, User, Settings, Moon, Sun, Globe, MessageSquareQuote, Send, Facebook, Youtube, Twitter, Radio, Instagram, ShieldCheck, Menu as MenuIconLucide, PackageIcon, ArrowLeft, LayoutDashboard, MessageSquareText, Users as UsersIcon, BookOpen, Headphones, Library as LibraryIcon, Search as SearchIconLucide } from 'lucide-react';
import {
  SidebarProvider,
  Sidebar,
  SidebarHeader,
...
const ThemeToggle = () => {
  ...
  useEffect(() => {
    ...
    const storedLang = (localStorage.getItem('language') as 'en' | 'ar') || 'ar';
...
const LanguageToggle = ({ onLanguageChange }: { onLanguageChange: (lang: 'en' | 'ar') => void }) => {
  const [mounted, setMounted] = useState(false);
  ...
  }, [currentLang, mounted, onLanguageChange]);
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" className="w-8 h-8" title={langToggleLabels[currentLang].title}>
          <Globe className="h-4 w-4" />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end">
        <DropdownMenuLabel>{langToggleLabels[currentLang].label}</DropdownMenuLabel>
        <DropdownMenuSeparator />
        <DropdownMenuItem onClick={() => setCurrentLang('en')} disabled={currentLang === 'en'}>
          English
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => setCurrentLang('ar')} disabled={currentLang === 'ar'}>
          العربية (Arabic)
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
...
function AppShellInternal({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === 'language' && event.newValue && (event.newValue === 'en' || event.newValue === 'ar')) {
        const newLang = event.newValue as 'en' | 'ar';
  ...
  return (
    <>
      ...
      <SidebarInset className="flex flex-col" dir={currentLanguage === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8" key={`${currentLanguage}-${pathname}`}>
          {children}
        </main>
...
function forceArabicLanguage() {
  ...
  document.documentElement.dir = (localStorage.getItem('language') || defaultLanguage) === 'ar' ? 'rtl' : 'ltr';
  
  if ((localStorage.getItem('theme') || defaultTheme) === 'dark') {
    document.documentElement.classList.add('dark');
  } else {
    document.documentElement.classList.remove('dark');
  }
