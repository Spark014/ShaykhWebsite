...
export default function AudioLibraryPage() {
  ...
  const renderAudioCard = (res: Resource) => (
    <Card key={`audio-${res.id}`} className="backdrop-blur-md bg-background/70 border border-border/30 shadow-lg hover:shadow-xl transition-all duration-300 rounded-xl">
      <CardHeader>
          ...
          <div className="flex gap-4 flex-1">
            <div className="relative w-20 h-20 bg-muted/50 backdrop-blur-sm rounded-xl overflow-hidden flex-shrink-0 border border-border/20">
              ...
              <div className={`cover-placeholder w-full h-full flex items-center justify-center ${res.cover_image_url ? 'hidden': ''}`} title={T.resourceIconAlt}>
                <CassetteTape className="h-8 w-8 text-muted-foreground/70" />
              </div>
            </div>
            <div className="flex-1 min-w-0">
              <CardTitle className="text-lg font-semibold text-primary font-headline line-clamp-2 leading-tight mb-1" title={res.title}>
                {res.title}
              </CardTitle>
              {res.collection?.name && (
                <Link href={`/library/collections/${res.collection_id}`} className="text-xs text-accent hover:underline font-medium block mb-2" title={res.collection.name}>
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                <span><strong>{T.categoryLabel}:</strong> {publicResourceCategories.find(rc => rc.value === res.category)?.label[currentLanguage] || res.category}</span>
                <span><strong>{T.languageLabel}:</strong> {resourceLanguagesList.find(rl => rl.value === res.language)?.label[currentLanguage] || res.language}</span>
              </div>
            </div>
          </div>
          <div className="flex flex-col gap-2 flex-shrink-0">
            <Button asChild size="sm" className="bg-primary hover:bg-primary/90 text-primary-foreground">
              <Link href={res.url} target="_blank" rel="noopener noreferrer">
                <ExternalLink className={`${currentLanguage === 'ar' ? 'ml-1 h-3 w-3' : 'mr-1 h-3 w-3'}`} />
                {T.listenAudioButton}
              </Link>
            </Button>
          </div>
        ...
        {res.tags && res.tags.length > 0 && (
          <div className="mb-3">
            <div className="flex flex-wrap gap-1.5">
              {res.tags.slice(0, 8).map(tag => (
                <Badge key={tag} variant="outline" className="text-xs font-normal bg-accent/10 text-accent-foreground/80 border-accent/30">
                  {tag}
                </Badge>
              ))}
              {res.tags.length > 8 && <Badge variant="outline" className="text-xs font-normal">+{res.tags.length - 8} more</Badge>}
            </div>
          </div>
        )}
  ...
  return (
    <div className="space-y-8 w-full" dir={currentLanguage === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <Tabs 
        ...
        className="w-full"
      >
        <div className="sticky bg-background/95 backdrop-blur-sm py-100 mb-6">
