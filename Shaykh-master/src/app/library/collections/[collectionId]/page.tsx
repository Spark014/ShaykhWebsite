  ...
  if (!collection && !isLoading) { 
    return (
      <div className="flex flex-col justify-center items-center py-12 space-y-4 min-h-[calc(100vh-200px)]">
        ...
        <Button onClick={() => router.push(collection?.collection_content_type === 'audio' ? '/library/audio' : '/library/books')} variant="outline">
          <ArrowLeft className={`${currentLanguage === 'ar' ? 'ml-2' : 'mr-2'} h-4 w-4`} /> {collection?.collection_content_type === 'audio' ? T.backToAudioLibrary : T.backToBookLibrary}
        </Button>
      </div>
    );
  }
