...
export default function QuestionsManagementPage() {
  ...
  const T = useMemo(() => pageTexts[currentLanguage], [currentLanguage]);
  ...
  const fetchQuestions = useCallback(async (statusFilter: 'all' | 'pending' | 'answered' | 'rejected', manualRefresh = false) => {
    if (authLoading || !currentUser) {
        console.warn("Admin Questions Management: fetchQuestions prerequisites not met (authLoading or no currentUser). Aborting API fetch.");
    ...
    try {
      ...
      const response = await fetch(`/api/admin/questions?status=${statusFilter}`, {
        headers: { 'Authorization': `Bearer ${token}` },
      });

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: "Failed to parse error response from API." }));
        console.error("API error fetching questions:", response.status, errorData);
      ...
      if (data && data.length === 0) {
         console.warn(`Admin Questions Management: No questions found from API for filter '${statusFilter}'.`);
      }

    } catch (error: any) {
      console.error("Error fetching questions via API:", error);
      toast({ title: T.loadError, description: error.message || T.fetchApiError, variant: "destructive", duration: 10000 });
      setQuestions([]);
    } finally {
      if (manualRefresh) setIsManuallyRefreshing(false); else setIsLoading(false);
    }
  }, [currentUser, T, toast, authLoading]);
  ...
  }, [authLoading, currentUser, filterStatus, fetchQuestions]);
                    ...
                    <div className="flex justify-between items-start">
                        <div>
                            <CardTitle className="text-lg font-semibold text-primary">
                                {adminResourceCategories.find(c => c.value === q.category)?.label[currentLanguage] || q.category}
                            </CardTitle>
